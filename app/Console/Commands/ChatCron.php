                ...
                ->chunk(10, function($requests) use($client){
                    foreach ($requests as $key=>$request_data) {
                        try{
                            ...
                            if($client->domain_name=='curenik')
                            {
                                $user = \App\User::where('id',$request_data->to_user)->first();
                                $seconds = (($user->slot_duration)?$user->slot_duration:$slot_duration->value)*60;
                            }
                            ...
                            if(strtotime($datenow)>=$next_hour_time){
                                if($request_data->requesthistory->status=='pending' && $client->domain_name!=='nurselynx'){
                                    ...
                                    if($client->domain_name!=='intely' && $client->domain_name!=='clouddoc' && $client->domain_name!=='nurselynx'){
