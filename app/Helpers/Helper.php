...
class Helper{
	...
	// Put the full path to your .pem file
        ...
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="taradoc"){
            $pem_file_name = "taradoc.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="clouddoc"){
            $pem_file_name = "clouddoc.pem";
        }
        ...
        $options = [
            'certificate_path' => $pemFile, // Path to private key
        ...
        $alert = Alert::create()->setTitle($params['message']);
        $alert = $alert->setBody($params['message']);
        ...
        $payload->setCustomValue('data', $params);
        ...
        foreach ($deviceTokens as $deviceToken) {
            if($deviceToken)
                $notifications[] = new PushNoti($payload,$deviceToken);
        }
        if(count($notifications)>0){
            $client = new PushClient($authProvider, $production = true);
    ...
    */
    public static function getDistance($latitudeFrom, $longitudeFrom,$latitudeTo,$longitudeTo, $unit = ''){
        $f_keys = self::getClientFeatureKeys('keys','Google');
        $origin = $latitudeFrom.",".$longitudeFrom;
        $destination = $latitudeTo.",".$longitudeTo;
        $api = file_get_contents("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=".$origin."&destinations=".$destination."&key=".$f_keys['map_key']);
  ...
  public static function getUserSubPackage($params){
    ...
    public static function getMoreData($user){
        ...
        $user->clinic_addresses = \App\Model\Office::getClinics($user->id);
        $user->user_slot_duration = null;
        if($user->slot_duration){
            $user->user_slot_duration = $user->slot_duration * 60;
        }else{
            $slot_duration = \App\Model\EnableService::where('type','slot_duration')->first();
            if($slot_duration)
                $user->user_slot_duration = $slot_duration->value * 60;
        }
    ...
    public static function createPaymentByAlRajahiBank($input,$user){
        try{
            $keys = self::getClientFeatureKeys('Payment Gateway','Al Rajhi Bank');
            $requires_source_action = true;
