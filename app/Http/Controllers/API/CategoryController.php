...
class CategoryController extends Controller
{
     ...
     *         in="query",
     *         type="number",
     *         description="0 or 1 default is 0",
     *         required=false,
     *     ),
     *     @SWG\Response(
     *         response=200,
     ...
     */
     ...
     public  function getCategories(Request $request) {
        try{
            ...
            return response(['status' => "success", 'statuscode' => 200,
                                'message' => __('Category Listing'), 'data' =>['classes_category'=>$parentCategories]], 200);
            ...
            return response(['status' => "success", 'statuscode' => 200,
                                'message' => __('Category Listing'), 'data' =>['classes_category'=>$parentCategories->items(),'after'=>$after,'before'=>$before,'per_page'=>$per_page]], 200);
        }
        }catch(Exception $ex){
            return response(['status' => "error", 'statuscode' => 500, 'message' => $ex->getMessage()], 500);
        }
     ...
     *         in="query",
     *         type="string",
     *         description=" type curenik,doctor",
     *         required=true,
     *     ),     
     *  @SWG\Parameter(
     *         name="category_id",
     ...
     */
     ...
     public  function getPackages(Request $request) {
        try{
            ...
            $package_from = isset($request->package_from)?$request->package_from:null;
            if($request->list_by=='purchased'){
                ...
                $packages = UserPackage::select('package_id','available_requests')->where('available_requests','>',0)->where(['user_id'=>$user->id])
                    ->whereHas('package', function($query) use ($request,$filter_id,$package_from){
                        if(isset($request->doctor_id)){
                            $query->where('created_by', $request->doctor_id);
                            $query->orWhere('created_by', null);
                        }
                        if($package_from=='admin' || $package_from=='doctor'){
                            $query->where('created_from_user', $package_from);
                        }
                ...
                foreach ($packages as $package) {
                 ...
                 if(config('client_connected') && (Config::get("client_data")->domain_name == "curenik"))
                    ...
                    if($user && $user->hasRole('service_provider'))
                     { 
                        $packages = Package::select('id','title','description','price','image','total_requests','category_id','package_type','created_by','filter_id', 'created_from_user')->where('category_id',$request->category_id)->where('filter_id',$filter_id)
                        ->where([
                            ['package_type','=',$request->type],
                            ['created_by', '=', $user->id],
                            ['package_created_for', '=', $request->usertype],
                            ['created_from_user', '=','doctor']
                            
                        ])->where('enable','1')->orderBy('id',"DESC")->cursorPaginate($per_page);
                    }
                ...
                else
                 {
                   ...
                   // ->orwhere('package_created_for' , $request->usertype)
                    ->orderBy('id',"DESC")
                    ->cursorPaginate($per_page);
                 ...
                 if($user){
                    foreach ($packages as $package) { 
                        ...
                        { 
                            ...
                            {
                                $userpackage = Package::where(['id'=>$package->package_id,'user_id'=>$user->id])->first();
                            }
                ...
                $packages = Package::select('id','title','description','price','image','total_requests','category_id','package_type','created_by','filter_id')->where('category_id',$request->category_id)->where('filter_id',$filter_id)
                    ->where('enable','1')
                    ->where(function($query) use($package_from){
                        if(config('client_connected') && (Config::get("client_data")->domain_name == "curenik")){
                            $query->where([
                                ['created_by','!=',null],
                                ['created_from_user', '=', 'doctor'],
                            ])->orwhere([
                                ['package_created_for','=','curenik'],
                                ['created_from_user', '=', 'admin'],
                                ['created_by', '=', null],
                            ]);
                            if($package_from=='admin' || $package_from=='doctor'){
                                $query->where('created_from_user', $package_from);
                ...
                { 
                    if($user && $user->hasRole('service_provider'))
                    {
                        $packages = Package::select('id','title','description','price','image','total_requests','category_id','package_type','created_by','created_from_user','filter_id')->where('category_id',$request->category_id)->where('filter_id',$filter_id)
                        ->where('package_type',$request->type)
                        ->where('package_created_for', $request->usertype)
                        ->where('created_from_user', 'admin')
                        ->where(function($query) use($package_from){
                            if($package_from=='admin' || $package_from=='doctor'){
                                $query->where('created_from_user', $package_from);
                            }
                    ...
                    }else if($user && $user->hasRole('customer'))
                    {
                        ...
                        { 
                            $packages = Package::select('id','title','description','price','image','total_requests','category_id','package_type','created_by','created_from_user','filter_id','package_created_for')->where('category_id',$request->category_id)->where('filter_id',$filter_id)
                            ->where('package_type',$request->type)
...
/**
                        ...
                        if($SpAdditionalDetail){
                            ...
                            if($SpAdditionalDetail->status!="approved" && isset($document['is_edit'])){
                                ...
                                if(Config('client_connected') && (Config::get("client_data")->domain_name=="care_connect_live"))
                                {
                                    ...
                                    $getuser->account_verified = null;
                                    $getuser->account_rejected = null;
                                    $getuser->save();
                                }
                            }
                        }else{
    ...
    public static function getClasses(Request $request){
    	try{
            ...
            $per_page = (isset($request->per_page)?$request->per_page:10);
            $classes = ConsultClass::select('id','name','description','status','calling_type','booking_date as class_date','created_at','booking_date as bookingDateUTC','price','category_id','created_by','duration','limit_enroll')
			...
			foreach ($classes as $key => $class) {
                ...
                $class->category_data->parent_name = null;
                if($class->category_data->parent){
                    $class->category_data->parent_name = $class->category_data->parent->name;
                    unset($class->category_data->parent);
                }
    ...
    public static function getClassDetail(Request $request){
        try{
            ...
            $per_page = (isset($request->per_page)?$request->per_page:10);
            $class = ConsultClass::select('id','name','description','status','calling_type','booking_date as class_date','created_at','booking_date as bookingDateUTC','price','category_id','created_by','duration','limit_enroll')
            ->where('id',$request->class_id)->first();
            $date = Carbon::parse($class->booking_date,'UTC')->setTimezone($timezone);
            $class->booking_date = $date->isoFormat('D MMMM YYYY, h:mm:ss a');
            $class->time = $date->isoFormat('h:mm a');
            $class->created_by = User::select('name','email','id','profile_image')->where('id',$class->created_by)->first();
            $class->category_data = Category::where('id',$class->category_id)->first();
            $class->category_data->parent_name = null;
            if($class->category_data->parent){
                $class->category_data->parent_name = $class->category_data->parent->name;
                unset($class->category_data->parent);
            }
            if($class->category_data->subcategory->count() > 0){
                $class->category_data->is_subcategory = true;
            }else{
                $class->category_data->is_subcategory = false;
            }
            $class->totalAssignedUser = $class->enroll_users->count();
            $class->isOccupied = $class->isOccupied($class->id,$user->id);
            if($class->enroll_users){
