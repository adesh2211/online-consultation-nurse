...
use App\Model\EnableService,App\Model\Request as Booking;
...
class DataController extends Controller
{
    ...
    public function getClientDetail(Request $request){
        ...
        if (Redis::exists('client_feature_'.$app_id) && $cache_enable) {
            ...
            $data = $this->getClientDetailFromDB($app_id,$user_type);
    ...
    public function getClientDetailFromDB($app_id,$user_type='') {
        $data = ['client_features'=>[],'domain'=>'default'];
        $data['currency_code'] = 'INR';
        $data['jitsi_id'] = 202001;
        $app_types = ['both'];
        if($user_type){
            $app_types = ['both',$user_type]; 
        }
        $data['pages'] = Page::select('slug','title','app_type')->whereIn('app_type',$app_types)->get();
        $data['need_master_preference'] = false;
        if(MasterPreference::count()>0){
            $data['need_master_preference'] = true;
        }
        $insurance = false;
        $country_id = null;
        $country_name = null;
        ...
        ->first();
        $data['domain_url'] = "https://royoconsult.com";
        $data['media_url'] = "https://consultants3assets.sfo2.digitaloceanspaces.com/";
        $data['socket_url'] = "https://node.royoconsult.com/";
        $data['jitsi_meet_url'] = "https://meet.royoapps.com/";
        $data['support_url'] = null;
         if($client){
            $data['domain'] = $client->domain_name;
            $data['domain_url'] = "https://$client->domain_name.royoconsult.com";
            if($client->domain_name=='healtcaremydoctor'){
                $data['support_url'] = "https://$client->domain_name.royoconsult.com/support";
            }
            if($client->domain_name=='intely'){
                $data['domain_url'] = "https://icareconnect.ca";
            }
            $data['payment_type'] = 'stripe';
            $data['gateway_key'] = '';
            $data['gateway_secret'] = '';
            // if($client->domain_name=='physiotherapist'){
            //     $data['payment_type'] = 'razor pay';
            //     $data['gateway_key'] = 'rzp_test_vM7sjDiwBmmTUC';
            //     $data['gateway_secret'] = 'CaqEcHGQJIGgJRIUnUJRHAwz';
            // }
            // if($client->domain_name=='healtcaremydoctor'){
            //     $data['payment_type'] = 'razor pay';
            //     $data['gateway_key'] = 'rzp_test_rc3VZ05BlafdKE';
            //     $data['gateway_secret'] = 'fFnJIlooDojvc6HosSpECP7w';
            // }
            // $country = Country::where('sortname',$client->country_code)->first();
            // if($country){
            ...
            foreach ($client_features as $key => $client_feature) {
                if($client_feature->feature_values){
                    ...
                    foreach ($client_feature->feature_values as $key_id => $value) {
                        ...
                        ->first();
                        if($featurekey){
                            if(strtolower($client_feature->feature->name)=='razor pay' && ($client->domain_name=='healtcaremydoctor' || $client->domain_name=='physiotherapist' || $client->domain_name=='curenik')){
                                $data[$featurekey->key_name] = $value;
                            }
                            if(strtolower($client_feature->feature->name)=='al rajhi bank' && ($client->domain_name=='homedoctor')){
                                $data['payment_type'] = 'al_rajhi_bank';
    ...
    public static function getHomePageData(Request $request){  
        try{
            ...
            $second_opinion_count = Booking::where('to_user',$user_id)->whereBetween('booking_date', [$from_date, $end_date])->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->where('request_type','second_oponion')->count();

            $chat_count = Booking::where('to_user',$user_id)->whereBetween('booking_date', [$from_date, $end_date])->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending']);
            })->whereHas('servicetype', function($query) {
                return $query->where('type','chat')->orWhere('service_type','chat');
        
            })->count();
            $audio_count = Booking::where('to_user',$user_id)->whereBetween('booking_date', [$from_date, $end_date])->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending']);
            })->whereHas('servicetype', function($query){
                return $query->where('type','audio_call')->orWhere('service_type','audio_call');
        
            })->count();

            $video_count = Booking::where('to_user',$user_id)->whereBetween('booking_date', [$from_date, $end_date])->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending']);
            })->whereHas('servicetype', function($query){
                return $query->where('type','video_call')->orWhere('service_type','video_call');
        
            })->count();
            $home_visit_count = Booking::where('to_user',$user_id)->whereBetween('booking_date', [$from_date, $end_date])->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending']);
            })->whereHas('servicetype', function($query) {
                return $query->where('type','home_visit')->orWhere('service_type','home_visit');
        
            })->count();

            $clinic_visit = Booking::where('to_user',$user_id)->whereBetween('booking_date', [$from_date, $end_date])->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending']);
            })->whereHas('servicetype', function($query) {
                return $query->where('type','clinic_visit')->orWhere('service_type','clinic_visit');
        
            })->count();

            $future_chat_count = Booking::where('to_user',$user_id)->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->whereHas('servicetype', function($query) {
                return $query->where('type','chat')->orWhere('service_type','chat');
        
            })->where('booking_date','>',$end_date)->count();
            $future_audio_count = Booking::where('to_user',$user_id)->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->whereHas('servicetype', function($query){
                return $query->where('type','audio_call')->orWhere('service_type','audio_call');
            ...
            $future_video_count = Booking::where('to_user',$user_id)->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->whereHas('servicetype', function($query){
                return $query->where('type','video_call')->orWhere('service_type','video_call');
        
            })->where('booking_date','>',$end_date)->count();
            $future_home_visit_count = Booking::where('to_user',$user_id)->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->whereHas('servicetype', function($query) {
                return $query->where('type','home_visit')->orWhere('service_type','home_visit');
            ...
            $future_clinic_visit = Booking::where('to_user',$user_id)->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->whereHas('servicetype', function($query) {
                return $query->where('type','clinic_visit')->orWhere('service_type','clinic_visit');
            ...
            $future_second_opinion_count = Booking::where('to_user',$user_id)->whereHas('requesthistory',function($query){
                return $query->whereIn('status',['in-progress','pending','accept']);
            })->where('request_type','second_oponion')->where('booking_date','>',$end_date)->count();
            ...
            $emsats =\App\Model\Emsat::select('*','icon as image')->orderBy('id', 'desc')->take(5)->get();
            ...
            return response(array(
                ...
                'data'=>[
                    ...
                    'emsats' => $emsats,
                    'count_data'=>[
                        'second_opinion_count'=>$second_opinion_count,
                        ...
                        'future_chat_count'=>$future_chat_count,
                        'future_audio_count'=>$future_audio_count,
                        'future_video_count'=>$future_video_count,
                        'future_home_visit_count'=>$future_home_visit_count,
                        'future_clinic_visit'=>$future_clinic_visit,
                        'future_second_opinion_count'=>$future_second_opinion_count,
                    ]
                ]), 200); 
        }catch(Exception $ex){
            return response(['status' => "error", 'statuscode' => 500, 'message' => $ex->getMessage()], 500);
    ...
    public function getMasterPreferences(Request $request,\App\Model\MasterPreference $masterpreference){
        ...
        if(strtolower($input['type'])=='all'){
            $MasterPreferences = $masterpreference->select('id','name as preference_name','is_multi','type as preference_type','show_on','is_required','input_type','data_type')->whereIn('show_on',$show_on)->where('created_by',null)->orderBy('id','ASC')->get();
        }else{
            $MasterPreferences = $masterpreference->select('id','name as preference_name','is_multi','type as preference_type','show_on','is_required','input_type','data_type')->where('created_by',null)->orderBy('id','ASC')->where('name',$input['type'])->whereIn('show_on',$show_on)->orderBy('id','ASC')->get();
        }
        if($MasterPreferences->count()>0){
            foreach ($MasterPreferences as $key => $MasterPreference) {
                ...
                if($MasterPreference->input_type!=='textbox'){
                    $MasterPreference->data_type = null;
                    ...
                    $exist = UserMasterPreference::where([
                        'user_id'=>$user_id,
                        'preference_id'=>$MasterPreference->id
                    ])->first();
                    $MasterPreference->input_value = null;
                    if($exist){
                        $MasterPreference->input_value = $exist->input_value;
                    }
                    $MasterPreference->options = [];
                }
            }
        }
        return response(['status' => "success",
