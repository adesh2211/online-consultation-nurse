...
class UserController extends Controller {
    ...
    public static function socialLogin(Request $request) {
        try{
          ...
          // $test_key = env('STRIPE_TEST_KEY');
            ...
            if(isset($request->is_agreed)){
                if($request->is_agreed===true){
                    $user->is_agreed = 1;
                }else{
                    $user->is_agreed = 0;
                }
     ...
     *         in="query",
     *         type="string",
     *         description="Clinic Logo image name",
     *         required=false,
     *     ),
     *     @SWG\Parameter(
     *         name="signature",
     *         in="query",
     *         type="string",
     *         description="Signature image name",
     *         required=false,
     *     ),
     *     @SWG\Parameter(
     *         name="profile_video",
     *         in="query",
     *         type="string",
     *         description="Profile Video link name",
     *         required=false,
     *     ),
     *     @SWG\Parameter(
     *         name="is_agreed",
        ...
        try{
            ...
            // if (is_object($smsVerifcationdata)) {
                ...
                if($user){
                    ...
                    if(isset($request->is_agreed)){
                        if($request->is_agreed==true || $request->is_agreed=='true'){
                            $user->is_agreed = 1;
                        }else{
                            $user->is_agreed = 0;
                        }
     ...
     *         in="query",
     *         type="string",
     *         description="Profile Video Name",
     *         required=false,
     *     ),
     *     @SWG\Parameter(
     *         name="is_agreed",
            ...
            if ($request->hasfile('image')) {
                if ($image = $request->file('image')) {
                    ...
                    if(isset($request->type) && strtolower($request->type)=='pdf'){
                        ...
                        $image_name = $filename;
                    }else if(isset($request->type) && strtolower($request->type)=='video'){
                        $FileEnconded=  \File::get($request->image);
   ...
   /**
    ...
    /**
     * @SWG\Get(
     *     path="/gethealthrecordslist",
     *     description="Get All Health Records List",
     * tags={"Customer"},
     *     security={
     *     {"Bearer": {}},
     *   },
    ...
    public function getHealthRecordsList(Request $request)
    {
        ...
        try{
            ...
            $healthRecordsList = HealthRecords::where('user_id',$user->id)->orderBy('id', 'asc')->cursorPaginate($per_page);

            $after = null;
            if($healthRecordsList->meta['next']){
                $after = $healthRecordsList->meta['next']->target;
            }
            $before = null;
            if($healthRecordsList->meta['previous']){
                $before = $healthRecordsList->meta['previous']->target;
            }
            $per_page = $healthRecordsList->perPage();
                
            return response(["status" => "success", 'statuscode' => 200, 'message'=>"Health Records List Found",'data' =>['lists'=>$healthRecordsList->items(),'after'=>$after,'before'=>$before,'per_page'=>$per_page]], 200);
        }catch (\Exception $e) {
            return response(["status" => "error", 'statuscode' => 500, 'message' => $e->getMessage()], 500);
        }
    }
