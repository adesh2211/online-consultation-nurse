...
use Exception;
use Config;
use App\Model\EmergencyTimeSlot;
...
class ServiceTypeController extends Controller
{
    ...
    public function editCategoryToService(Category $category,$id)
    {
        ...
        $emergencytimeslots = EmergencyTimeSlot::where('category_sevice_type',$categoryservicetype->id )->get();
       // return json_encode($emergencytimeslots);
        return view('admin.services.categories.edit',compact('category','categoryservicetype','emergencytimeslots'));
    }
    ...
    public function putCategoryToService(Category $category,$id,Request $request)
    {
    	...
    	$validator = \Validator::make($request->all(),$rules,$custom_message);
          ...
          if(Config('client_connected') && Config::get("client_data")->domain_name=="curenik"){
            if(isset($input['start_time']) && isset($input['end_time']) && $input['price'])
            {
                if($input['start_time'] != '' && $input['start_time'] != null && $input['end_time'] != '' && $input['end_time'] != null && $input['price'] != '' && $input['price'] != null )
                {

                $end_time = $input['end_time'];
                $price = $input['price'];
                
                $emergencyslot = $input['emergency_slot_id'];
                
                //remove old
                if (is_array($emergencyslot) || is_object($emergencyslot))
                { 
                    foreach($emergencyslot as $key=>$slot){
                        $delEmergency = EmergencyTimeSlot::where('id',$slot)->where('category_sevice_type', $id)->delete();
                    }
                }

                //add new
                foreach($input['start_time'] as $key=>$start)
                {
                            
                            $emergency = new EmergencyTimeSlot();
                            $emergency->category_sevice_type = $categoryservicetype->id;
                            $emergency->start_time = $start;
                            $emergency->end_time = $end_time[$key];
                            $emergency->price = $price[$key];
                            $emergency->save();
    ...
    public function postCategoryToService($service_id,Request $request)
    {
     ...
     /**
     * Show the form for delete emergency slots time the specified resource.
     *
     * @param  \App\Model\EnableService  $enableService
     * @return \Illuminate\Http\Response
     */

    public function deleteEmergencySlot(Request $request)
    {
       
        if($request->slotId != '' || $request->slotId != Null)
        {
            $emergency  = EmergencyTimeSlot::where('id',$request->slotId)->delete();
            return response(['status' => 'success','message' => 'EmergencySlot Delete Successfully' ]);
        }
    }
    ...
    public function createCategoryToService(Category $category,Request $request)
    {
          ...
          $categoryservicetype->price_maximum = $range[1];
          $categoryservicetype->save();
       //  print_r($categoryservicetype->id); die();
          if(Config('client_connected') && Config::get("client_data")->domain_name=="curenik"){
            if(isset($input['start_time']) && isset($input['end_time']) && $input['price'])
            {
              if($input['start_time'] != '' && $input['start_time'] != null && $input['end_time'] != '' && $input['end_time'] != null && $input['price'] != '' && $input['price'] != null )
