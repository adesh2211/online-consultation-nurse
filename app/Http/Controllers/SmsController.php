...
Android app download Link:https://drive.google.com/drive/folders/16B2wfuNUAYWA1A6EX6qSQr3QIrpVeyL6"; 
    ...
    public function sendSms($request) {
        ...
        if(\Config::get('client_connected') && (\Config::get('client_data')->domain_name!=='healtcaremydoctor' && \Config::get('client_data')->domain_name!=='curenik' && \Config::get('client_data')->domain_name!=='physiotherapist' && \Config::get('client_data')->domain_name!=='intely' && \Config::get('client_data')->domain_name!=='nurselynx' && \Config::get('client_data')->domain_name!=='airdoc' && \Config::get('client_data')->domain_name!=='clouddoc')){
        ...
        try {
            ...
            if(\Config::get('client_connected') && (\Config::get('client_data')->domain_name=='healtcaremydoctor')){
                ...
                $body = "Here's your CloudDoc verification code: $request->code. This code will expire in 15 minutes.";
            }
            $twilio = new Client($accountSid, $authToken);
            try{
