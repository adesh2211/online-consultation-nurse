...
class Transaction extends Model
{
     ...
     */
    protected $fillable = [
        'amount', 'transaction_type', 'status','wallet_id','closing_balance','request_id','class_id','transaction_id','payment_gateway','module_table','module_id'
    ];
    ...
    public static function createWithdrawal($transaction_detail){
		...
		if($transaction_detail['status']=='succeeded'){
            ...
            if($transaction->amount>0){
                ...
                $notification->message =__('notification.booking_amount_de_text2');
                $notification->save();
                $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'BOOKING_RESERVED','message'=>__('notification.booking_amount_de_text2')));
            }
    ...
    public static function createWithdrawalUberLike($transaction_detail){
        $transaction = self::create(array(
                'amount'=>$transaction_detail['balance'],
                'transaction_type'=>'withdrawal',
                'status'=>'pending',
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $payment = \App\Model\Payment::create(array(
                'from'=>$transaction_detail['from_id'],
                'to'=>$transaction_detail['user']->id,
