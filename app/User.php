...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'name', 'email','user_name', 'password','phone','fcm_id','profile_image','email_verified_at','country_code','insurance_enable','manual_available','npi_id','source','profile_video','clinic_logo','signature'
    ];
    ...
    public function getIsAgreedAttribute($value) {
        if($value){
            return true;
        }else{
            return false;
    ...
    */
    ...
    public function spcourse()
    {
        return $this->hasMany('App\Model\SpCourse','sp_id');
    }
    public function emsat()
    {
        return $this->hasMany('App\Model\SpEmsat','sp_id');
    }
    ...
    */
    ...
    public static function getDoctorDetail($id){
        ...
        if($user && $user->hasRole('service_provider')){
            ...
            $user_slot_duration = null;
            if($user->slot_duration){
                $user_slot_duration = $user->slot_duration * 60;
            }else if($slot_duration){
                $user_slot_duration = $slot_duration->value * 60;
            }
            $user->user_slot_duration = $user_slot_duration;
            ...
            $user->clinics = \App\Model\Office::getClinics($user->id);
            $user->totalRating = 0;
            if($user->profile){
            ...
            //$user->packages = $user->getPackage($user);
   ...
   */
    ...
    * @param 
    */
    public static function getServicesByServiceProvider($sp_id,$service_id){
        $subscriptions = [];
        $user = self::where('id',$sp_id)->first();
        $cat = \App\Model\CategoryServiceProvider::where('sp_id',$sp_id)->first(); 
        if(!$cat){
         ...
         $categoryservicetype = CategoryServiceType::where('category_id',$cat->category_id)->where('service_id',$service_id)->pluck('id')->toArray();
        if(count($categoryservicetype)<1){
            return $subscriptions;
        }
        $subscriptions = \App\Model\SpServiceType::where([
